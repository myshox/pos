  ...
  return (
    <div className="min-h-[60vh] flex items-center justify-center p-6">
      <div className="w-full max-w-sm card-market rounded-2xl p-6 sm:p-8">
        <h2 className="text-xl font-semibold text-rose-800 mb-2 text-center">
          {showSetForm ? t('pinSetTitle') : t('pinEnterTitle')}
        </h2>
        <p className="text-rose-500 text-sm text-center mb-6">
        ...
        {showSetForm ? (
          <form onSubmit={handleSetPin} className="space-y-4">
            <label className="block">
              <span className="text-sm text-rose-600">{t('pinNew')}</span>
              <input
                type="password"
                inputMode="numeric"
                autoComplete="off"
                ...
                placeholder="••••"
                className="mt-1 w-full border border-rose-300 rounded-xl px-4 py-3 text-center text-lg tracking-widest min-h-[48px]"
                maxLength={8}
              />
            </label>
            <label className="block">
              <span className="text-sm text-rose-600">{t('pinConfirm')}</span>
              <input
                type="password"
                inputMode="numeric"
                autoComplete="off"
            ...
            {adminHasPin && mode === 'set' && (
              <button type="button" onClick={() => setMode('enter')} className="w-full py-2 text-rose-500 text-sm">
                {t('cancel')}
              </button>
            )}
          </form>
          ...
          <form onSubmit={handleUnlock} className="space-y-4">
            <input
              ...
              placeholder="••••"
              className="w-full border border-rose-300 rounded-xl px-4 py-3 text-center text-lg tracking-widest min-h-[48px]"
              maxLength={8}
            />
            <button type="submit" className="btn-primary w-full py-3 rounded-xl font-medium min-h-[48px]">
              {t('pinUnlock')}
            </button>
            <button type="button" onClick={() => setMode('set')} className="w-full py-2 text-rose-500 text-sm">
              {t('pinChange')}
            </button>
          </form>
        )}
