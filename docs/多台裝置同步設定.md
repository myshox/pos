...
2. **先** `npm run build`（或 `npm run build:cap`），**再** `npx cap sync`／開啟 Xcode／Android Studio 打包。  
   - 若先打包過「沒有環境變數」的 `dist`，裝到手機上的 App **不會**連 Supabase，看起來就像「iOS 與安卓無法同步」。
3. 改過 `.env` 後務必重新 `npm run build` 再同步原生專案。
4. App 內可到 **後台 → 備份與還原**：若沒出現「已啟用雲端同步」區塊，代表該次建置未帶入 `VITE_*`。
